...
| フェーズ名 (`this.phase`) | 画面の状態 | 監視対象・トリガー | 遷移先フェーズ | 備考 |
| :--- | :--- | :--- | :--- | :--- |
| **`WAITING_MATCHING`** | 対戦マッチング待機中 | 画面左下の選出ボタンボールマーク (`COORDS.MATCHING_BALL`) のテンプレート一致度 > 0.45 | `MATCHING` | 対戦開始前の待機状態 |
| **`MATCHING`** | 見せ合い画面（手持ち6体と選出選択） | 初回フレームで相手6体・トレーナー名を自動取得。<br>その後ボールマークの一致度 < 0.35 になるまで本フェーズを維持 | `WAITING_GAME_START` | ボールマークが消える（暗転）まで絶対に試合中へフライングしない |
| **`WAITING_GAME_START`** | 見せ合い終了〜対戦開始演出 | 中央VSロゴ (`COORDS.VS_SCREEN`) 一致度 > 0.38、または見せ合い終了から 7000ms 経過 | `IN_GAME` | 出撃演出のボール投げシーンを即座に監視可能にする |
| **`IN_GAME`** | 対戦中（出撃演出、コマンド画面、交代） | 出撃ネームプレート、常時表示HPバー、様子を見るパネルを監視して自分・相手の選出（最大4体）を取得。<br>勝敗ボール検知で試合終了判定 | `FINISHED` | 先発2体だけでなく、交代・繰り出し（3〜4体目）も自動追跡 |
| **`FINISHED`** | 勝敗確定画面 | 勝敗（勝ち／負け）を記録し、対戦ログを保存 | `WAITING_MATCHING` | 次の対戦へ向けて自動リセット |
